...
(ert-deftest test-terraform-mode--assignment ()
  (dolist (case '(((description . "assignment target inside locals block")
                  ...
                  ((description . "hyphenated key in map gets variable-name face")
                   (content     . "locals {\nregion_map = {\nus-east-1 = \"US East\"\n}\n}")
...
(ert-deftest test-terraform-mode--resource-sub-block ()
  (dolist (case '(((description . "sub-block label inside resource block gets variable-name face")
                  ...
                  ((description . "sub-block outside resource block gets no variable-name face")
                   ...
                   (check       . (((pos . 1) (face . nil)))))
                  ((description . "sub-block label inside provider block gets variable-name face")
                   (content     . "provider \"aws\" {\ndefault_tags {\n}\n}")
                   (check       . (((pos . 18) (face . font-lock-variable-name-face)))))
                  ((description . "sub-block outside provider block gets no variable-name face")
...
(ert-deftest test-terraform-mode--reference-keywords ()
  (dolist (case '(((description . "var gets builtin face inside resource block")
                  ...
                  ((description . "self reference gets builtin face inside resource block")
                   (content     . "resource \"r\" \"n\" {\nself.public_ip\n}")
                   (check       . (((pos . 21) (face . font-lock-builtin-face)))))
                  ((description . "ephemeral reference gets builtin face inside resource block")
                   (content     . "resource \"r\" \"n\" {\nephemeral.aws_secret.foo.value\n}")
                   (check       . (((pos . 21) (face . font-lock-builtin-face)))))
                  ((description . "var inside connection sub-block gets builtin face")
                   (content     . "resource \"r\" \"n\" {\nconnection {\nhost = self.public_ip\n}\n}")
                   (check       . (((pos . 40) (face . font-lock-builtin-face)))))
...
(ert-deftest test-terraform-mode--for-expression ()
  (dolist (case '(((description . "for keyword gets builtin face")
                  ...
                  ((description . "var gets variable-name face in multi-line for expression")
                   (content     . "[\n  for log in list : log\n]")
                   (check       . (((pos . 9) (face . font-lock-variable-name-face)))))
                  ((description . "inner for-var body usage gets variable-name face in nested for expression")
                   (content     . "[for k, v in map : [for x in v : x]]")
                   (check       . (((pos . 34) (face . font-lock-variable-name-face)))))
                  ((description . "inner for-var declaration gets variable-name face in nested for expression")
                   (content     . "[for k, v in map : [for x in v : x]]")
                   (check       . (((pos . 25) (face . font-lock-variable-name-face)))))
                  ((description . "for-var after dot in chained access does not get variable-name face")
                   (content     . "[for rule in list : rule.rule.rule.rule]")
                   (check       . (((pos . 21) (face . font-lock-variable-name-face))
                                   ((pos . 26) (face . nil)))))
                  ((description . "for-var at start of chained access gets variable-name face")
                   (content     . "[for rule in list : rule.rule.rule.rule]")
                   (check       . (((pos . 21) (face . font-lock-variable-name-face)))))))
    (terraform-test-face (alist-get 'description case)
                         (alist-get 'content case)
...
(ert-deftest test-terraform-mode--doc-commands-scoping ()
  "All doc commands enforce block scoping via terraform-mode--doc-url-at-point."
  (dolist (cmd '(terraform-mode-open-doc
                 terraform-mode-copy-doc-url
    ...
    (dolist (case '(((description . "errors with guard message inside variable block")
                    ...
                    ((description . "succeeds inside resource block")
      ...
      (terraform-test-with-buffer (alist-get 'content case)
        ...
        (if (alist-get 'expected-error case)
          ...
          (should (progn (cl-letf (((symbol-function 'browse-url) #'ignore))
