...
(ert-deftest test-terraform-mode--builtin-functions ()
  (dolist (case '(((description . "merge gets builtin face")
                   (content     . "merge(var.a, var.b)")
                   (check       . (((pos . 1) (face . font-lock-builtin-face)))))
                  ((description . "tostring gets builtin face")
                   (content     . "tostring(42)")
                   (check       . (((pos . 1) (face . font-lock-builtin-face)))))
...
(ert-deftest test-terraform-mode--ternary ()
  (dolist (case '(((description . "? gets builtin face")
                   (content     . "var.enabled ? 1 : 0")
                   (check       . (((pos . 13) (face . font-lock-builtin-face)))))
                  ((description . ": gets builtin face")
                   (content     . "var.enabled ? 1 : 0")
                   (check       . (((pos . 17) (face . font-lock-builtin-face)))))))
    (terraform-test-face (alist-get 'description case)
                         (alist-get 'content case)
                         (alist-get 'check case))))

(ert-deftest test-terraform-mode--literal-keywords ()
  (dolist (case '(((description . "true gets builtin face globally")
                   (content     . "true")
                   (check       . (((pos . 1) (face . font-lock-builtin-face)))))
                  ((description . "false gets builtin face globally")
                   (content     . "false")
                   (check       . (((pos . 1) (face . font-lock-builtin-face)))))
                  ((description . "true inside resource block gets builtin face")
                   (content     . "resource \"aws_instance\" \"foo\" {\ndelete_on_termination = true\n}")
                   (check       . (((pos . 57) (face . font-lock-builtin-face)))))
                  ((description . "false inside resource block gets builtin face")
                   (content     . "resource \"aws_instance\" \"foo\" {\ndelete_on_termination = false\n}")
                   (check       . (((pos . 57) (face . font-lock-builtin-face)))))
                  ((description . "null gets builtin face")
                   (content     . "null")
                   (check       . (((pos . 1) (face . font-lock-builtin-face)))))))
    (terraform-test-face (alist-get 'description case)
                         (alist-get 'content case)
...
(ert-deftest test-terraform-mode--reference-keywords ()
  (dolist (case '(((description . "var gets builtin face")
                  ...
                  ((description . "module reference gets builtin face")
                   ...
                   (check       . (((pos . 1) (face . font-lock-builtin-face)))))
                  ((description . "data reference gets builtin face")
                   (content     . "data.aws_ami.ubuntu.id")
                   (check       . (((pos . 1) (face . font-lock-builtin-face)))))))
    (terraform-test-face (alist-get 'description case)
                         (alist-get 'content case)
                         (alist-get 'check case))))
...
(ert-deftest test-terraform-mode--resource-builtins ()
  (dolist (case '(((description . "for_each gets builtin face inside resource block")
                  ...
                  ((description . "count gets builtin face inside resource block")
                   (content     . "resource \"aws_instance\" \"foo\" {\ncount = 3\n}")
                   (check       . (((pos . 33) (face . font-lock-builtin-face)))))
                  ((description . "for_each outside resource block gets variable-name face not builtin")
                   (content     . "for_each = {}")
                   (check       . (((pos . 1) (face . font-lock-variable-name-face)))))))
    (terraform-test-face (alist-get 'description case)
                         (alist-get 'content case)
